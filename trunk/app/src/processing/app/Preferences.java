...
public class Preferences {
  ...
  static protected void init(String commandLinePrefs) {
    ...
    try {
      ...
      Base.showError(null, "Could not read default settings.\n" +
                           "You'll need to reinstall Karat.", e);
    }
  ...
  public Preferences() {
    ...
    label = new JLabel("  (requires restart of Karat)");
    box.add(label);
    pain.add(box);
    ...
    if (Base.isWindows()) {
      autoAssociateBox =
        new JCheckBox("Automatically associate .pde files with Karat");
    ...
    label = new JLabel("(edit only when Karat is not running)");
    pain.add(label);
    d = label.getPreferredSize();
    label.setForeground(Color.gray);
    label.setBounds(left, top, d.width, d.height);
